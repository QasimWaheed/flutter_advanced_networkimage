...
class AdvancedNetworkSvg extends SvgLoader {
  AdvancedNetworkSvg(
    this.url, {
    this.scale = 1.0,
    this.header,
  ...
  Future<AdvancedNetworkSvg> obtainKey(ImageConfiguration picture) {
    return SynchronousFuture<AdvancedNetworkSvg>(this);
  }

  ImageStreamCompleter load(AdvancedNetworkSvg key,
      {ImageErrorListener? onError}) {
    return OneFrameImageStreamCompleter(
      _loadAsync(key, onError: onError),
      informationCollector: () sync* {
        yield DiagnosticsProperty<SvgLoader>('Picture provider', this);
  ...
  Future<ImageInfo> _loadAsync(AdvancedNetworkSvg key,
