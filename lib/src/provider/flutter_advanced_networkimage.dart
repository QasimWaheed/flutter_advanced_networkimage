...
class AdvancedNetworkImage extends ImageProvider<AdvancedNetworkImage> {
  ...
  Future<ui.Codec> _loadAsync(
    ...
    if (useDiskCache) {
      try {
        ...
        if (_diskCache != null) {
          ...
          var buffer = await ImmutableBuffer.fromUint8List(_diskCache);
          if (key.loadedCallback != null) key.loadedCallback!();
          return decode(
            buffer,
            getTargetSize: (w,h) {
              return ui.TargetImageSize(width: key.width, height: key.height);
            },
    ...
    if (key.fallbackAssetImage != null) {
      ...
      var buffer = await ImmutableBuffer.fromUint8List(imageData.buffer.asUint8List());
      return decode(
        buffer,
        getTargetSize: (w,h) {
          return ui.TargetImageSize(width: key.width, height: key.height);
    ...
    if (key.fallbackImage != null) {
      var buffer = await ImmutableBuffer.fromUint8List(key.fallbackImage!);
      return decode(
        buffer,
        getTargetSize: (w, h) {
          return ui.TargetImageSize(width: key.width, height: key.height);
